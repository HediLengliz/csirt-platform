...
class ElasticDetector(BaseDetector):
    ...
    def normalize_event(self, raw_event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "timestamp": raw_event.get("@timestamp") or raw_event.get("timestamp", datetime.utcnow().isoformat()),
            "source_ip": raw_event.get("source.ip") or raw_event.get("src_ip") or raw_event.get("source_ip"),
            "destination_ip": raw_event.get("destination.ip") or raw_event.get("dest_ip") or raw_event.get("destination_ip"),
            "user": raw_event.get("user.name") or raw_event.get("user") or raw_event.get("username"),
            "hostname": raw_event.get("host.name") or raw_event.get("hostname") or raw_event.get("host"),
            "description": raw_event.get("message") or raw_event.get("description") or str(raw_event),
            "event_type": raw_event.get("event.type") or raw_event.get("event_type") or raw_event.get("action"),
            "severity_score": raw_event.get("event.severity") or raw_event.get("severity") or raw_event.get("priority"),
