...
class NetworkDetector(BaseDetector):
    ...
    def normalize_event(self, raw_event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "timestamp": raw_event.get("timestamp") or raw_event.get("time", datetime.utcnow().isoformat()),
            "source_ip": raw_event.get("src_ip") or raw_event.get("source_ip") or raw_event.get("src"),
            "destination_ip": raw_event.get("dest_ip") or raw_event.get("destination_ip") or raw_event.get("dst"),
            ...
            "description": raw_event.get("description") or raw_event.get("message") or raw_event.get("signature"),
            "event_type": raw_event.get("event_type") or raw_event.get("type") or raw_event.get("action"),
