...
def _get_event_context(db, event: Event) -> dict:
    ...
    if event.source_ip:
        context["source_ip_count"] = db.query(func.count(Event.id)).filter(
            and_(
        ...
        context["source_ip_count_24h"] = db.query(func.count(Event.id)).filter(
            and_(
                Event.source_ip == event.source_ip,
    ...
    if event.destination_ip:
        context["destination_ip_count"] = db.query(func.count(Event.id)).filter(
            and_(
    ...
    if event.user:
        context["user_count"] = db.query(func.count(Event.id)).filter(
            and_(
    ...
    if event.event_type:
        context["similar_events_count"] = db.query(func.count(Event.id)).filter(
            and_(
