...
class AlertPrioritizer:
    ...
    def extract_features(self, event: Event, context: Dict[str, Any] = None) -> np.ndarray:
        ...
        malware_keywords = ["malware", "virus", "trojan", "ransomware", "rootkit", "backdoor", "spyware"]
        suspicious_patterns = ["unauthorized", "breach", "exploit", "attack", "intrusion", "compromise"]
        exploit_keywords = ["exploit", "cve-", "vulnerability", "zero-day", "rce", "sql injection"]
        privilege_keywords = ["privilege", "escalation", "sudo", "admin", "root", "administrator"]
        ...
        # Source reliability
        source_reliability = self._calculate_source_reliability(event.source.value if hasattr(event.source, 'value') else str(event.source))
    ...
    def _calculate_intelligent_score(self, event: Event, context: Dict[str, Any] = None) -> float:
        ...
        min_score = max(0.10, event_type_severity * 0.5)  # At least 50% of base severity
    ...
    def predict_priority(self, event: Event, context: Dict[str, Any] = None) -> Tuple[AlertPriority, float]:
        ...
        if hasattr(self.model, "classes_") and len(self.model.classes_) > 0:
            try:
                ...
                priority = priority_mapping.get(predicted_class_idx, AlertPriority.MEDIUM)
    ...
    def _rule_based_priority(self, event: Event) -> AlertPriority:
        ...
        # Critical events
        if event.event_type in [EventType.MALWARE_DETECTED, EventType.UNAUTHORIZED_ACCESS, EventType.DATA_EXFILTRATION]:
            return AlertPriority.CRITICAL
        
        # High priority
        if event.event_type in [EventType.SUSPICIOUS_ACTIVITY, EventType.BRUTE_FORCE, EventType.DDoS]:
            return AlertPriority.HIGH
    ...
    def train(self, training_data: List[Dict[str, Any]]):
        ...
        print(f"Model training complete. Train accuracy: {train_score:.2f}, Test accuracy: {test_score:.2f}")
    ...
    def extract_features_from_dict(self, data: Dict[str, Any]) -> np.ndarray:
        ...
        class MockEvent:
            def __init__(self, data):
                ...
                self.source = type('obj', (object,), {'value': data.get("source", "custom")})()
